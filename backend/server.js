...
try {
  ...
  if (!firebaseConfig.project_id) throw new Error("No project_id in config");
  admin.initializeApp({ credential: admin.credential.cert(firebaseConfig) });
  db = admin.firestore();
  adminAuth = admin.auth();
  console.log("✅ Firebase Admin connected. Project ID:", firebaseConfig.project_id);
} catch (err) {
...
// Auth middleware – with detailed error
async function ensureAuthenticated(req, res, next) {
  ...
  try {
    ...
    res.status(401).json({ error: "Invalid token", details: err.message, code: err.code });
  }
}
...
app.get("/api/firebase-test", async (req, res) => {
  ...
  try {
    await db.collection("users").doc("test").set({ test: true, time: Date.now() }, { merge: true });
    res.json({ success: true, message: "Firebase connection OK" });
  } catch (err) {
...
function shouldPreserveHairstyle(promptText) {
  ...
  const keywords = ["change hair", "different hair", "new hair", "different hairstyle", "new hairstyle", "change hairstyle", "alter hair", "modify hair", "different haircut", "new haircut"];
...
app.post("/api/edit", ensureAuthenticated, upload.single("image"), async (req, res) => {
  try {
    ...
    const hairInstruction = preserveHair ? "preserve exact hairstyle" : "change hairstyle according to description";
    ...
    const result = await fal.subscribe("fal-ai/flux-pulid", {
      input: {
        ...
        prompt: `${prompt}, same exact person, preserve face, preserve eyes, preserve identity, ${hairInstruction}, ultra realistic, cinematic lighting, realistic human, DSLR photography, realistic skin texture, detailed face, masterpiece, 8k quality`,
      },
    });
